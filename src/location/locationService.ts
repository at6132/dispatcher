...
export async function getDriverLocation(): Promise<DriverLocation | null> {
  ...
  refreshInFlight = (async () => {
    try {
      ...
      if (stored && isUsableCachedLocation(stored)) return stored;
      ...
      if (nativeCached) {
        return publishLocation(fromLocationObject(nativeCached));
      }

      const currentFix = Location.getCurrentPositionAsync({
        accuracy: Location.Accuracy.Balanced,
      }).then((location) => publishLocation(fromLocationObject(location)));
